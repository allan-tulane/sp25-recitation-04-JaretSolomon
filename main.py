...
def word_count_reduce(group):
  """
    ...
    """
  return (group[0], reduce(lambda x, y: x + y, group[1][0], group[1][0:]))
  #return list(reduce(lambda x: (x, len(x[0][0])), group.split())


#print(word_count_reduce(['i', [1, 1, 1]]))  #== ('i', 3)
...
def sentiment_map(doc,
  ...
  docs = doc.split()
  tuples = run_map_reduce(
      word_count_map, word_count_reduce,
      docs)  # # returns list of tuples with ('term', frequency)

  pos_list = []
  neg_list = []
  for tup in tuples:
    if tup[0] in pos_terms:
      pos_list.append(('positive', tup[1]))

    elif tup[0] in neg_terms:
      neg_list.append(('negative', tup[1]))

  #return (group[0], reduce(lambda x, y: x + y, group[1][0], group[1][0:]))
  tmp = pos_list + neg_list
  return tmp


#print(
#    sentiment_map(
#        'it was a good corrupted good terrible waste of corrupted time'))
